...
const HistoricalForm = ({ selectedPollutant, searchedValue, onSubmit, onReset }) => {
  ...
  const fetchData = async (getDataFn) => {
    try {
      const response = await getDataFn(startDate, endDate, searchValue, currentPollutant);
      const results = response.results;
      const filteredData = results.filter(result => result.parameter === currentPolutantName);
