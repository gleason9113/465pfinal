...
export const getLatestCityData = async (city) => {
  try {
    const url = `https://api.openaq.org/v2/latest?limit=1&city=${encodeURIComponent(
      city
    )}`;
    const response = await fetch(url);
    if (!response.ok) {
...
export const getLatestCountryData = async (country) => {
  try {
    const url = `https://api.openaq.org/v2/latest?limit=1&country=${encodeURIComponent(
      country
    )}`;
    const response = await fetch(url);
    if (!response.ok) {
...
export const getDateRange = async (startDate, endDate, location, type) => {
  try {
    ...
    const url = `https://api.openaq.org/v2/measurements?date_from=${encodeURIComponent(
      startDate
    )}&date_to=${encodeURIComponent(endDate)}&${encodeURIComponent(
      type
